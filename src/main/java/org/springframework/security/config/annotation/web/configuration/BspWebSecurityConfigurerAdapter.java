...
@Order(100)
public class BspWebSecurityConfigurerAdapter implements
        WebSecurityConfigurer<WebSecurity> {

    private final Log logger = LogFactory.getLog(BspWebSecurityConfigurerAdapter.class);
    ...
    private BspHttpSecurity http;
    private boolean disableDefaults;

    /**
     * Creates an instance with the default configuration enabled.
     */
    protected BspWebSecurityConfigurerAdapter() {
        this(false);
    }
     ...
     * false
     */
    protected BspWebSecurityConfigurerAdapter(boolean disableDefaults) {
     ...
     * {@link BspHttpSecurity#getSharedObject(Class)} for use with other
    ...
    /**
     * Creates the {@link BspHttpSecurity} or returns the current instance
     *
     * ] * @return the {@link BspHttpSecurity}
     * @throws Exception
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    protected final BspHttpSecurity getHttp() throws Exception {
        if (http != null) {
            return http;
        }
        ...
        http = new BspHttpSecurity(objectPostProcessor, authenticationBuilder,
                sharedObjects);
        if (!disableDefaults) {
            // @formatter:off
            http
     ...
     */
    public AuthenticationManager authenticationManagerBean() throws Exception {
        return new BspWebSecurityConfigurerAdapter.AuthenticationManagerDelegator(authenticationBuilder, context);
    }
    ...
    public UserDetailsService userDetailsServiceBean() throws Exception {
        ...
        return new BspWebSecurityConfigurerAdapter.UserDetailsServiceDelegator(Arrays.asList(
    ...
    public void init(final WebSecurity web) throws Exception {
        final BspHttpSecurity http = getHttp();
     ...
     * {@link #configure(BspHttpSecurity)} and the {@link BspHttpSecurity#authorizeRequests}
    ...
    /**
     * Override this method to configure the {@link BspHttpSecurity}. Typically subclasses
     ...
     * See {@link BspHttpSecurity#authorizeRequests} and the `permitAll()` authorization rule
     * for more details on public endpoints.
     *
     * @param http the {@link BspHttpSecurity} to modify
    ...
    public void setApplicationContext(ApplicationContext context) {
        ...
        BspWebSecurityConfigurerAdapter.LazyPasswordEncoder passwordEncoder = new BspWebSecurityConfigurerAdapter.LazyPasswordEncoder(context);

        authenticationBuilder = new BspWebSecurityConfigurerAdapter.DefaultPasswordEncoderAuthenticationManagerBuilder(objectPostProcessor, passwordEncoder);
        localConfigureAuthenticationBldr = new BspWebSecurityConfigurerAdapter.DefaultPasswordEncoderAuthenticationManagerBuilder(objectPostProcessor, passwordEncoder) {
            @Override
